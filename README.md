...
make storybook    # templ generate 後にコンポーネントカタログを起動
make test         # go test ./...
```
...
- `make storybook` starts the `templ/storybook` component catalog at `http://localhost:60606`.
- Storybook uses the tracked config in `storybook-server/` and installs frontend dependencies there on first run. `node_modules/`, generated stories, and static build output are ignored.
